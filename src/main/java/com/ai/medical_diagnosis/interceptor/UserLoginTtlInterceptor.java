...
@Slf4j
@Component
public class UserLoginTtlInterceptor implements HandlerInterceptor {

    private static StringRedisTemplate stringRedisTemplate;

    public UserLoginTtlInterceptor(StringRedisTemplate stringRedisTemplate) {
        UserLoginTtlInterceptor.stringRedisTemplate = stringRedisTemplate;
    }
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (!mapUser.isEmpty()){
            UserInfoDTO user = BeanUtil.fillBeanWithMap(mapUser, new UserInfoDTO(), false);
            user.setToken(token);
//            log.info("用户登录状态：{}", user);
            // 2.1 用户已登录，保存用户
